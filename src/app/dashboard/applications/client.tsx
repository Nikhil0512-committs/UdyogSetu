...
export default function ApplicationsClient({ initialApplications }: { initialApplications: Application[] }) {
  ...
  const inReviewCount = initialApplications.filter(
    (a) => a.status === "IN_REVIEW" || a.status === "SUBMITTED"
  ).length;
  const actionRequiredCount = initialApplications.filter(
    (a) => a.status === "QUERIED"
  ).length;
  const approvedCount = initialApplications.filter(
    (a) => a.status === "APPROVED"
  ).length;
  const draftCount = initialApplications.filter(
    (a) => a.status === "DRAFT"
  ).length;
  ...
  }, [initialApplications, searchTerm, statusFilter, departmentFilter, sortBy]);

  const departmentsList = React.useMemo(() => {
    const list = Array.from(new Set(initialApplications.map((a) => a.deptShort)));
    return list;
  }, [initialApplications]);
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 text-sm mt-1">Track status, respond to queries, and access granted certificates.</p>
          </div>
        </div>
        <div className="text-center py-16 bg-white border border-slate-200 rounded-xl shadow-sm">
          <FileText className="w-16 h-16 text-slate-300 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-slate-900">No applications found</h3>
          <p className="text-slate-500 max-w-sm mx-auto mt-2 mb-6">You haven't submitted any applications for your enterprise yet.</p>
          <Link href="/dashboard/new">
            <Button className="bg-[#b3401a] hover:bg-[#923315] text-white font-semibold">
        ...
        <CardContent className="p-4">
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
            ...
            <div className="relative flex-1">
              ...
              <Input
                type="text"
                placeholder="Search by ID, approval name, or department..."
                value={searchTerm}
